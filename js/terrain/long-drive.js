...
/**
 * Long Drive Generator - Infinite procedural flat terrain with variations
 * 
 * Features:
 * - Infinite chunk-based terrain generation (Same logic as IceMountain)
 * - Flat with uneven ground (No steep downhill)
 * - Standard surface types (Same as Terrain)
 */
export class LongDriveGenerator extends BasePhysicsProvider {
    constructor(params = {}) {
        super();

        this.noise = new PerlinNoise(params.seed || 12345);
        this.ridgeNoise = new PerlinNoise((params.seed || 12345) + 1000);
        ...
        // Terrain parameters
        this.peakHeight = 80;                          // Height of boundary peaks
        this.mountainSpacing = 150;                    // Width of the boundary mountains
        this.ridgeOffset = 400;                        // Distance from center to side ridges (Wider for long drive)
    ...
    _updateChunks(playerPosition) {
        ...
        for (let dz = -this.chunksPerSide; dz <= this.chunksPerSide; dz++) {
            for (let dx = -3; dx <= 3; dx++) { // Wider X range since the valley is wider
    ...
    _calculateHeight(worldX, worldZ) {
        // Flat base (no downhill slope)
        let adjustedBase = 0;

        // Uneven ground: Slight variations
        const unevenness = this.noise.fbm(worldX * 0.005, worldZ * 0.005, 3, 2, 0.5) * 5.0;
        adjustedBase += unevenness;

        // Add larger rolling variations (hills but flat-ish)
        const rollingHills = this.noise.noise2D(worldX * 0.001, worldZ * 0.001) * 10.0;
        adjustedBase += rollingHills;

        // Calculate distance from center (X = 0)
        const absX = Math.abs(worldX);

        // Twin mountain ridges to bound the level
        let ridgeHeight = 0;

        // Left ridge (negative X) - Boundary
        ...
        if (leftRidgeDist < this.mountainSpacing) {
            ...
            ridgeHeight = Math.max(ridgeHeight, ridgeFactor * ridgeFactor * this.peakHeight * (0.8 + 0.4 * peakNoise));
        }

        // Right ridge (positive X) - Boundary
    ...
    /**
     * Calculate vertex color - mimicking terrain.js palette
     */
    _calculateColor(worldX, worldZ, height) {
        // Color palette mimicking terrain.js
        ...
        const colorNoise = this.noise.noise2D(worldX * 0.05, worldZ * 0.05) * 0.3;

        // Use local height relative to base (approx 0) to determine color
        // But since we have hills, let's just use absolute height somewhat
        const relativeHeight = height + colorNoise * 5;

        let color = new THREE.Color();

        // Simple height-based coloring
        if (relativeHeight < 2) {
            // Low/Ground -> Dirt/Grass mix
            color.lerpColors(dirt, grassLow, THREE.MathUtils.clamp((relativeHeight + 2) / 4, 0, 1));
        } else if (relativeHeight < 15) {
            // Hills -> Grass
            color.lerpColors(grassLow, grassHigh, THREE.MathUtils.clamp((relativeHeight - 2) / 13, 0, 1));
        } else {
            // Peaks/Ridges -> Rock
            color.lerpColors(grassHigh, rock, THREE.MathUtils.clamp((relativeHeight - 15) / 20, 0, 1));
        }
    ...
    getSpawnPosition() {
        const spawnX = 0;
        const spawnZ = 0;
        const spawnY = this._calculateHeight(spawnX, spawnZ) + 2;
    ...
    /**
     * Get surface type - Similar to terrain.js
     */
    getSurfaceType(worldX, worldZ) {
        ...
        if (height < 2) {
            return SurfaceTypes.DIRT || { friction: 0.6, drag: 0.01, type: 'dirt' };
        } else if (height < 15) {
            return SurfaceTypes.GRASS || { friction: 0.6, drag: 0.02, type: 'grass' };
        } else {
            return SurfaceTypes.GRAVEL || { friction: 0.5, drag: 0.02, type: 'gravel' };
        }
    }

    /**
     * Get gravity - Standard
     */
    getGravity() {
        return 9.81;
    }
